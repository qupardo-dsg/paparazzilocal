    ...
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center" onClick={onClose}>
      <div className="bg-white rounded-xl shadow-xl max-w-[520px] w-[90%] max-h-[85vh] overflow-y-auto p-6" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="grid grid-cols-2 gap-3 mb-5 p-4 bg-[var(--color-surface)] rounded-lg">
          <div>
            <p className="text-xs text-[var(--color-muted)] uppercase tracking-wider">Pedido</p>
            <p className="text-sm font-mono font-semibold">{order.id}</p>
          </div>
          <div>
            <p className="text-xs text-[var(--color-muted)] uppercase tracking-wider">Cliente</p>
            <p className="text-sm font-semibold">{order.customer}</p>
          </div>
          <div>
            <p className="text-xs text-[var(--color-muted)] uppercase tracking-wider">Fecha</p>
            <p className="text-sm">{order.date}</p>
          </div>
          <div>
            <p className="text-xs text-[var(--color-muted)] uppercase tracking-wider">Estado</p>
            <StatusBadge status={order.status} />
          </div>
          <div>
            <p className="text-xs text-[var(--color-muted)] uppercase tracking-wider">Productos</p>
            <p className="text-sm font-semibold">{order.items}</p>
          </div>
          <div>
            <p className="text-xs text-[var(--color-muted)] uppercase tracking-wider">Total</p>
        ...
        <div className="space-y-2">
          {order.lineItems.map((li, i) => {
            ...
            return (
              <div
                key={i}
                className="flex items-center gap-3 p-3 bg-[var(--color-surface)] border border-[var(--color-border-soft)] rounded-lg"
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold truncate">
                    {p ? p.name : `Producto #${li.productId}`}
                  </p>
                  {p && <p className="text-xs text-[var(--color-meta)]">{p.category} · SKU: {p.sku}</p>}
                </div>
                <div className="text-right shrink-0">
                  <p className="text-sm text-[var(--color-muted)]">x{li.quantity}</p>
                  <p className="text-sm font-semibold">${formatPrice(li.price)}</p>
                </div>
              </div>
            );
          })}
        ...
        <div className="flex justify-between items-center mt-5 pt-4 border-t border-[var(--color-border-soft)]">
          <span className="text-sm text-[var(--color-muted)]">{order.items} producto{order.items !== 1 ? "s" : ""}</span>
